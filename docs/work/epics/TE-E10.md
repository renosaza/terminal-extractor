...
Промежуточный локальный Stop и grant epoch реализованы в [TE-T038](../tasks/TE-T038.md). Синтетическая проверка подтвердила отзыв при блокированном export action и прекращение обмена через текущие client sockets; живой Stop во время Ghostty export не проверен. Menu-bar UI и privacy mode отсутствуют; bounded Ghostty screen read существует только с отдельным локальным согласием.
