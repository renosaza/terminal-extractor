...
| AT-03 / FR-03 | Открыть 3 окна, вкладки и Ghostty split; показать список и явно выбрать одну строку; повторить с одинаковыми titles, без выбора, после смены focus, rename/reorder и close/recreate | Каждая доступная сессия различима; без выбора нет attach; ввод не уходит соседу; closed/reused binding даёт STALE_GENERATION/TARGET_NOT_FOUND; no fallback |
...
| AT-09 / FR-09 | REPL, SSH в тестовый host, less/vim, prompts, no-Enter paste, Tab/arrows/Escape/Ctrl+C | Managed работает интерактивно в том же pane; native отклоняет неподдерживаемое до dispatch; nested input не получает fake local exit code |
