...
| AT-03 / FR-03 | Открыть 3 окна, вкладки и Ghostty split; переключать focus, переименовывать, переставлять; закрыть и создать похожий target | Ввод не уходит соседу; closed/reused binding даёт STALE_GENERATION/TARGET_NOT_FOUND; no fallback |
