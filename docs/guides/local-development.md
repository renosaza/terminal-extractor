...
`python3 -B probes/tmux/read_only_clients.py` проверяет на двух синтетических PTY-клиентах private tmux, что `attach-session -f read-only,ignore-size` блокирует их ввод в fixture pane, а обычный клиент сохраняет ввод. Это не GUI-проверка и не product handoff.
