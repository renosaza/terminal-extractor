...
`python3 -B probes/tmux/read_only_clients.py` проверяет на двух синтетических PTY-клиентах private tmux, что `attach-session -f read-only,ignore-size` блокирует ввод второго клиента в fixture pane и не уменьшает window 80×24 при его размере 20×10; обычный клиент сохраняет ввод. Это не GUI-проверка и не product handoff.

`swift build`, затем `swift run TermexVisibleOwnerCheck --self-check` проверяют тот же `ManagedTmux` owner/gate/guarded launch на временной detached pane; GUI при этом не используется. Для отдельной живой проверки `TermexVisibleOwnerCheck` запускается только самим пользователем в явно выбранной тестовой вкладке Terminal.app без `--self-check`: fixture сверяет её TTY с Terminal.app до и после отдельного локального подтверждения нового managed shell. Отмена прекращает проверку до создания private pane. После подтверждения fixture временно накладывает новую private tmux shell, показывает синтетический marker и ждёт `tmux detach-client` в её prompt. Физическое `Ctrl-B, D` в живом запуске 2026-09-25 не дало detach; причина не установлена. После detach fixture проверяет marker в private capture segment, metadata и exact binding; это не product view binding и не перенесение исходной истории. Кроме `tmux detach-client`, не вводить команды или секреты внутри fixture: до 4096 байт временно записываются в private каталог 0700. При обычном exact close и `clean_eof` каталог удаляется; при неопределённом отказе сохраняется для ручного разбора, без автоматического `kill-server`. Если напечатан `private_fixture_retained`, не удалять каталог до выяснения состояния private session; для продолжения достаточно сообщить факт отказа и путь, не содержимое.

При нажатии `Cancel` fixture печатает только `consent_cancelled=PASS private_owner=NOT_CREATED` и не создаёт pane; это отдельная отрицательная GUI-проверка, а не успешный attach.
